...
// Hardcoded programs data (same as in Peradeniya_programs.php)
$hardcoded_programs = [
...
// Count hardcoded programs
$hardcoded_counts = [];
foreach ($hardcoded_programs as $level => $categories) {
    $count = 0;
    foreach ($categories as $category => $programs) {
        $count += count($programs);
    }
    $hardcoded_counts[$level] = $count;
}

// Fetch Peradeniya programs from database
try {
    $stmt = $pdo->prepare("
        SELECT 
            level,
        ...
        WHERE campus = 'University of Peradeniya' AND status = 'Active'
        GROUP BY level
        ORDER BY 
            CASE level
                WHEN 'Certificate & Advanced Certificate' THEN 1
                WHEN 'Diploma' THEN 2
                WHEN 'Professional Short Courses' THEN 3
                ELSE 4
            END
    ");
    $stmt->execute();
    ...
    $default_levels = [
        'Certificate & Advanced Certificate' => [
            'count' => 0,
            'description' => 'Comprehensive certificate programs designed to provide specialized skills and foundational knowledge in various disciplines.',
            'programs' => []
        ],
        'Diploma' => [
            'count' => 0,
            'description' => 'Advanced diploma programs offering in-depth professional development and specialized training.',
            'programs' => []
        ],
        'Professional Short Courses' => [
            'count' => 0,
            'description' => 'Intensive short-term courses focused on practical skills and professional development.',
            'programs' => []
        ]
    ];
    ...
    // Add hardcoded program counts to database counts
    foreach ($study_levels as $level => $data) {
        $hardcoded_count = $hardcoded_counts[$level] ?? 0;
        $study_levels[$level]['count'] += $hardcoded_count;
    }
    ...
    $study_levels = [
        'Certificate & Advanced Certificate' => [
            'count' => $hardcoded_counts['Certificate & Advanced Certificate'] ?? 0,
            'description' => 'Comprehensive certificate programs designed to provide specialized skills and foundational knowledge in various disciplines.',
            'programs' => []
        ],
        'Diploma' => [
            'count' => $hardcoded_counts['Diploma'] ?? 0,
            'description' => 'Advanced diploma programs offering in-depth professional development and specialized training.',
            'programs' => []
        ],
        'Professional Short Courses' => [
            'count' => $hardcoded_counts['Professional Short Courses'] ?? 0,
            'description' => 'Intensive short-term courses focused on practical skills and professional development.',
            'programs' => []
        ]
    ];
...
function getLevelDescription($level) {
    $descriptions = [
        'Certificate & Advanced Certificate' => 'Comprehensive certificate programs designed to provide specialized skills and foundational knowledge in various disciplines.',
        'Diploma' => 'Advanced diploma programs offering in-depth professional development and specialized training.',
        'Professional Short Courses' => 'Intensive short-term courses focused on practical skills and professional development.'
    ];
    
    return $descriptions[$level] ?? 'Professional development programs for career advancement';
}
?>
...
<head>
    ...
    <title>University of Peradeniya - Study Levels - EduConnectSL</title>
...
<body>
    ...
    <section class="study-levels-content">
        <div class="container">
            ...
            <div class="page-title">
                <h2>
                    <span class="university-name">University of Peradeniya</span> - <span class="study-levels-text">Study Levels</span>
                </h2>
            ...
            <div class="study-levels-grid">
                <?php 
                $card_images = [
                    'Certificate & Advanced Certificate' => 'img/peradeniya program/certificate .jpeg',
                    'Diploma' => 'img/peradeniya program/diploma.jpg',
                    'Professional Short Courses' => 'img/peradeniya program/shortcourses.jpg'
                ];
                foreach ($study_levels as $level => $data): 
                ?>
                <div class="study-level-card" onclick="selectStudyLevel('<?php echo $level; ?>')">
                    <div class="card-image" style="background-image: url('<?php echo $card_images[$level]; ?>');"></div>
    ...
    <script>
        function selectStudyLevel(studyLevel) {
            ...
            window.location.href = `Peradeniya_programs.php?level=${encodeURIComponent(studyLevel)}`;
        }
